...
__global__ void gpu__convolve_2d(
    ...
    //TODO: load the weights and input channel into shared memory,
    //for loading the input channel, you can do this all on a single thread (when the 
    //output channel is 0) for each input channel in the batch
     // Load weights into shared memory
    if (batch == 0) {
        for (int i = 0; i < n_convolution_weights; ++i) {
    ...
    if (output_channel == 0) { 
        for (int i = 0; i < in_channel_width; ++i) {
