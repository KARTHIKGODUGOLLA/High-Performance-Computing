...
SelfOrganizingMap::SelfOrganizingMap(vector<string> _labels, uint32_t _sample_size, uint32_t _height, uint32_t _width, int32_t seed, uint32_t _batch_size) : labels(_labels), sample_size(_sample_size), width(_height), height(_width), batch_size(_batch_size) {
    random_number_generator = mt19937(seed);

    initialize_cells();
}

SelfOrganizingMap::SelfOrganizingMap(vector<string> _labels, uint32_t _sample_size, uint32_t _height, uint32_t _width, uint32_t _batch_size) : labels(_labels), sample_size(_sample_size), height(_height), width(_width), batch_size(_batch_size) {
    random_device rd;
...
void SelfOrganizingMap::train(Dataset *dataset, uint32_t epochs, int32_t neighbor_radius, double learning_rate, double learning_rate_schedule) {
    ...
    for (uint32_t epoch = 0; epoch < epochs; epoch++) {
        ...
        for(int32_t i=0;i<dataset->samples.size();i+=batch_size)
        {
            std::vector<std::thread> batch_threads;
            for(int32_t j=0;j<batch_size && (i+j)<dataset->samples.size();j++)
            {
                const Sample *sample =dataset->samples[i+j];

                batch_threads.emplace_back([&, sample]()
                                          {
                    CellIndex bmu_index = get_best_matching_unit(sample);

                // Get the BMUs neighbors
                std::vector<CellIndex> neighbor_indexes;
                get_neighbors(bmu_index, neighbor_radius, neighbor_indexes);

                // Lock neighbors and update them
                std::vector<std::unique_lock<std::mutex>> locks;
                for (const CellIndex &index : neighbor_indexes) {
                    locks.emplace_back(cells[index.y][index.x]->cell_mutex);
                }
                for (const CellIndex &neighbor_index : neighbor_indexes) {
                    update_cell(sample, bmu_index, neighbor_index, learning_rate);
                } });
            }
            // Join all threads in the batch before moving to the next batch
        ...
        // Decrease learning rate according to schedule
