...
int main(int argc, char** argv) {
    ...
    if (arguments.size() != 9) {
        ...
        cerr << "./" << executable_name << " <dataset_filename> <som_width> <som_height> <learning_rate> <learning_rate_schedule> <neighbor_radius> <epochs> <batch_size>" << endl;
        exit(1);
    }
    ...
    int32_t batch_size = stoi(arguments[8]);
    ...
    SelfOrganizingMap *som = new SelfOrganizingMap(dataset->labels, dataset->sample_size(), som_width, som_height,batch_size);

    som->train(dataset, epochs, neighbor_radius, learning_rate, learning_rate_schedule);
